...
public class HDBSCAN implements Clusterer, Parameterized {
    ...
    public int[] cluster(DataSet dataSet, boolean parallel, int[] designations) {
        ...
        @SuppressWarnings("unchecked") final List<Vec> X = dataSet.getDataVectors();
        final int N = X.size();
        ...
        while (!edgeQ.isEmpty()) {
            ...
            if (new_size >= m_clSize && a_size < m_clSize && b_size < m_clSize) {//birth of a new cluster!
                ...
                map_to_cluster_label[mergedClust] = next_cluster_label;
                next_cluster_label++;
            } else if (new_size >= m_clSize) {//existing cluster has grown in size, so add the points and record their weight for use later
