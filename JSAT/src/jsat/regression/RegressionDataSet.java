...
public class RegressionDataSet extends DataSet<RegressionDataSet> {
    ...
    public RegressionDataSet(List<DataPoint> data, int predicting) {
        ...
        for (DataPoint dp : data) {
            ...
            if (origV.isSparse()) newVec = new SparseVector(origV.length() - 1, origV.nnz());
            else newVec = new DenseVector(origV.length() - 1);

            for (IndexValue iv : origV)
                if (iv.getIndex() < predicting) newVec.set(iv.getIndex(), iv.getValue());
                else if (iv.getIndex() == predicting) target = iv.getValue();
    ...
    public static RegressionDataSet comineAllBut(List<RegressionDataSet> list, int exception) {
        ...
        for (int i = 0; i < list.size(); i++)
            ...
            else for (int j = 0; j < list.get(i).size(); j++)
