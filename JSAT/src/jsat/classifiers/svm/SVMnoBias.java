...
public class SVMnoBias extends SupportVectorLearner implements BinaryScoreClassifier {
    ...
    public SVMnoBias(SVMnoBias toCopy) {
        super(toCopy);
        if (toCopy.weights != null) this.weights = toCopy.weights.clone();
        if (toCopy.label != null) this.label = Arrays.copyOf(toCopy.label, toCopy.label.length);
        this.C = toCopy.C;
    ...
    public void setKernel(KernelTrick kernel) {
        if (kernel.normalized()) super.setKernel(kernel);
        else super.setKernel(new NormalizedKernel(kernel));
    }
    ...
    public CategoricalResults classify(DataPoint data) {
        if (vecs == null) throw new UntrainedModelException("Classifier has yet to be trained");

        CategoricalResults cr = new CategoricalResults(2);

        double sum = getScore(data);

        if (sum > 0) cr.setProb(1, 1);
        else cr.setProb(0, 1);
        ...
        ParallelUtils.run(parallel, N, (start, end) -> {
            ...
            for (int i = start; i < end; i++) {
                ...
                for (int j = 0; j < N; j++) {
                    ...
                    double k_ij;
                    if (getCacheMode() == CacheMode.FULL) k_ij = kEval(i, j);
                    else k_ij = k(i, j);
...
//    @Parameter.WarmParameter(prefLowToHigh = true)
    public void setC(double C) {
        if (C <= 0) throw new ArithmeticException("C must be a positive constant");
        this.C = C;
    }
